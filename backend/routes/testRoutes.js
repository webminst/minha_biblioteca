// Arquivo de rotas de teste para simular rate limiting e resetar tentativas
// ...código original copiado de test.js...
...
router.delete('/reset-rate-limit', async (req, res) => {
  try {
    ...
    if (!isRedisConnected()) {
      return res.status(503).json({ success: false, message: 'Redis não disponível' });
    }
    const keys = await redis.keys(`*${ip}*`);
    if (keys.length > 0) {
      await redis.del(...keys);
    }
    res.json({ success: true, message: `Rate limiting resetado para IP ${ip}`, removedKeys: keys.length });
  } catch (error) {
    console.error('Erro ao resetar rate limiting:', error);
    res.status(500).json({ success: false, message: 'Erro ao resetar rate limiting' });
  }
});
...
router.get('/check-attempts', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Erro ao verificar tentativas' });
  }
});
