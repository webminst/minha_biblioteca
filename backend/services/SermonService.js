    ...
    const suggestions = await Sermon.aggregate([
      ...
      },
      {
        $project: {
          ...
          score: {
            $add: [
              { $cond: [{ $regexMatch: { input: '$title', regex } }, 10, 0] },
              { $cond: [{ $regexMatch: { input: '$bibleReference', regex } }, 8, 0] },
              { $cond: [{ $regexMatch: { input: '$series', regex } }, 6, 0] },
              { $cond: [{ $regexMatch: { input: '$speaker', regex } }, 4, 0] },
              { $cond: [{ $regexMatch: { input: '$book', regex } }, 2, 0] },
            ],
          },
        },
      },
      ...
      { $limit: limit },
      {
        $project: {
          text: {
            $cond: {
              if: { $and: [{ $gt: ['$title', null] }, { $regexMatch: { input: '$title', regex } }] },
              then: '$title',
              else: {
                $cond: {
                  if: { $and: [{ $gt: ['$bibleReference', null] }, { $regexMatch: { input: '$bibleReference', regex } }] },
                  then: '$bibleReference',
                  else: {
                    $cond: {
                      if: { $and: [{ $gt: ['$series', null] }, { $regexMatch: { input: '$series', regex } }] },
                      then: '$series',
                      else: {
                        $cond: {
                          if: { $and: [{ $gt: ['$speaker', null] }, { $regexMatch: { input: '$speaker', regex } }] },
                          then: '$speaker',
                          else: '$book',
                        },
                      },
