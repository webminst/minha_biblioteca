...
export const useApi = (url, options = {}, dependencies = []) => {
  ...
  const extractDataByType = (responseData, url) => {
    if (options.dataType) {
      ...
      case 'sermons':
        return extractSermons(responseData);
      case 'studies':
        return extractStudies(responseData);
      case 'books':
        return extractBooks(responseData);
      default:
        return responseData;
      }
    }
  ...
  const mutate = useCallback(
    async (...args) => {
      try {
        ...
        const errorMessage =
          err.response?.data?.message || err.message || 'Erro na operação';
  ...
  const setStoredValue = useCallback(
    newValue => {
      try {
        ...
        console.error(
          `Erro ao salvar localStorage para chave "${key}":`,
          error,
        );
      }
    },
