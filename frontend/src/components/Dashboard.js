...
function Dashboard({ user }) {
  ...
  const fetchCounts = async () => {
    try {
      ...
      setCounts({
        sermons:
          (sermonsData.success ? sermonsData.data.count : sermonsData.count) ||
          0,
        studies:
          (studiesData.success ? studiesData.data.count : studiesData.count) ||
          0,
        books:
          (booksData.success ? booksData.data.count : booksData.count) || 0,
      });
    } catch (error) {
  ...
  return (
    <div className='dashboard-container'>
      ...
      {user && (
        <p>
          Bem-vindo(a), <strong>{user.username}</strong>! ({user.role})
        </p>
      )}
      ...
      <div className='dashboard-links'>
        <div className='dashboard-item'>
          ...
          <span className='item-count'>
            {loading
              ? 'Carregando...'
              : `${counts.sermons} sermão${counts.sermons !== 1 ? 'es' : ''} cadastrado${counts.sermons !== 1 ? 's' : ''}`}
          </span>
        </div>
        ...
        <div className='dashboard-item'>
          ...
          <span className='item-count'>
            {loading
              ? 'Carregando...'
              : `${counts.studies} estudo${counts.studies !== 1 ? 's' : ''} cadastrado${counts.studies !== 1 ? 's' : ''}`}
          </span>
        </div>
        ...
        <div className='dashboard-item'>
          ...
          <span className='item-count'>
            {loading
              ? 'Carregando...'
              : `${counts.books} livro${counts.books !== 1 ? 's' : ''} cadastrado${counts.books !== 1 ? 's' : ''}`}
          </span>
        </div>
        ...
        <div className='dashboard-item'>
          ...
          <span className='item-count'>Visualizar logs do sistema</span>
        </div>
        ...
        <div className='dashboard-item security-section'>
          ...
          <span className='item-count'>Autenticação de dois fatores</span>
        </div>
      </div>
      ...
      <div className='dashboard-stats'>
        ...
        <p>
          ...
          <strong>
            {loading ? '...' : counts.sermons + counts.studies + counts.books}
          </strong>
        </p>
      </div>
    </div>
