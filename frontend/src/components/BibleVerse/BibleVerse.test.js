...
describe('BibleVerse', () => {
    ...
    it('renderiza erro e botão de tentar novamente', async () => {
        ...
        render(<BibleVerse reference="João 3:16" />);
        await waitFor(() => expect(screen.getByText(/referência não encontrada/i)).toBeInTheDocument());
        expect(screen.getByRole('button', { name: /tentar novamente/i })).toBeInTheDocument();
    });

    it('renderiza manual load se autoLoad for false', () => {
        render(<BibleVerse reference="João 3:16" autoLoad={false} />);
        expect(screen.getByText(/clique para carregar o versículo/i)).toBeInTheDocument();
        expect(screen.getByRole('button', { name: /carregar/i })).toBeInTheDocument();
    });

    it('renderiza versículo e ações', async () => {
        fetchVerse.mockResolvedValue({ text: 'Porque Deus amou o mundo', reference: 'João 3:16', translation_name: 'Almeida' });
        render(<BibleVerse reference="João 3:16" />);
        await waitFor(() => expect(screen.getByText(/porque deus amou o mundo/i)).toBeInTheDocument());
    ...
    it('renderiza seletor de tradução se showTranslationSelector for true', async () => {
        fetchVerse.mockResolvedValue({ text: 'Texto', reference: 'João 3:16', translation_name: 'Almeida' });
        render(<BibleVerse reference="João 3:16" showTranslationSelector />);
        await waitFor(() => expect(screen.getByLabelText(/tradução/i)).toBeInTheDocument());
        expect(screen.getByRole('option', { name: /almeida/i })).toBeInTheDocument();
