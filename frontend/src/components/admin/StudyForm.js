...
function StudyForm() {
  ...
  const formatOptions = ['Estudo', 'Palestra', 'Conferência', 'Outros'];
    ...
    if (isEditing) {
      const fetchStudy = async () => {
        ...
        try {
          ...
          setDate(
            studyData.date
              ? new Date(studyData.date).toISOString().split('T')[0]
              : '',
          );
          ...
          setError(
            `Erro ao carregar dados do estudo: ${
      ...
      <form onSubmit={handleSubmit} className='admin-form'>
        ...
        <div className='form-group'>
          <label htmlFor='speaker'>
            Ministrante (opcional, padrão: 'Giovanni Guimarães'):
          </label>
          <input
            type='text'
            id='speaker'
        ...
        <Link to='/admin/estudos' className='btn-back'>
          Voltar para a Lista
        </Link>
      </form>
    </div>
