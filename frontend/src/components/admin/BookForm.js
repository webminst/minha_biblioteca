...
function BookForm() {
        ...
        if (isEditing) {
            const fetchBook = async () => {
                ...
                try {
                    ...
                    setPersonalRating(bookData.personalRating ? String(bookData.personalRating) : '');
                    setDifficulty(bookData.difficulty || '');
                    setPurchaseLinks(bookData.purchaseLinks ? bookData.purchaseLinks.map(l => `${l.store}: ${l.url}${l.price ? ` (${l.price})` : ''}`).join('\n') : '');
                    setIsPublished(bookData.isPublished !== undefined ? bookData.isPublished : true);
                    setFeatured(bookData.featured !== undefined ? bookData.featured : false);
                    setPublicationYear(bookData.publicationYear ? String(bookData.publicationYear) : '');
    ...
    const handleSubmit = async (e) => {
        ...
        if (!content || content.length < MIN_SUMMARY_LENGTH) {
            setContentError(`O conteúdo/resumo deve ter pelo menos ${MIN_SUMMARY_LENGTH} caracteres.`);
            setLoading(false);
            return;
        } else if (content.length > MAX_SUMMARY_LENGTH) {
            setContentError(`O resumo não pode ter mais de ${MAX_SUMMARY_LENGTH} caracteres. Atualmente tem ${content.length} caracteres.`);
            setLoading(false);
            return;
        } else {
        ...
        const bookData = {
            ...
            purchaseLinks: purchaseLinks ? purchaseLinks.split('\n').map(l => {
                const [storeUrl, price] = l.split('(');
                const [store, url] = storeUrl.split(':').map(s => s.trim());
                return store && url ? { store, url, price: price ? Number(price.replace(/\D/g, '')) : undefined } : null;
        ...
        try {
            ...
            if (err.response?.status === 400 && errorMessage.includes('Nenhum campo válido para atualizar')) {
                errorMessage = 'Nenhuma alteração detectada. Edite algum campo antes de salvar.';
            ...
            <form onSubmit={handleSubmit} className="admin-form">
                ...
                <div className="optional-fields">
                    <div className="form-group">
                        ...
                        <input type="text" id="author" value={author} onChange={e => setAuthor(e.target.value)} disabled={loading} required />
                    </div>
                    <div className="form-group">
                    ...
                    <div className="form-row">
                        <div className="form-col">
                            ...
                            <input type="text" id="isbn" value={isbn} onChange={e => setIsbn(e.target.value)} disabled={loading} />
                        </div>
                        <div className="form-col">
                            <label htmlFor="pageCount">Páginas</label>
                            <input type="number" id="pageCount" value={pageCount} onChange={e => setPageCount(e.target.value)} disabled={loading} min="1" />
                        </div>
                        <div className="form-col">
                            <label htmlFor="personalRating">Avaliação (1-5)</label>
                            <input type="number" id="personalRating" value={personalRating} onChange={e => setPersonalRating(e.target.value)} disabled={loading} min="1" max="5" />
                        </div>
                        {/* Removido campo 'Ano' duplicado */}
                    </div>
                    <div className="form-group">
                        <label htmlFor="difficulty">Dificuldade (opcional):</label>
                        <select id="difficulty" value={difficulty} onChange={e => setDifficulty(e.target.value)} disabled={loading}>
                    ...
                    <div className="form-group">
                        <label htmlFor="purchaseLinks">Links de Compra (um por linha, formato: Loja: URL (preço opcional)):</label>
                        <textarea id="purchaseLinks" value={purchaseLinks} onChange={e => setPurchaseLinks(e.target.value)} rows="3" disabled={loading}></textarea>
                    </div>
                    <div className="form-group">
                        <div className="form-row">
                            <div className="form-col">
                                <label htmlFor="isPublished">Publicado?</label>
                                <input type="checkbox" id="isPublished" checked={isPublished} onChange={e => setIsPublished(e.target.checked)} disabled={loading} />
                            </div>
                            <div className="form-col">
                                <label htmlFor="featured">Destaque?</label>
                                <input type="checkbox" id="featured" checked={featured} onChange={e => setFeatured(e.target.checked)} disabled={loading} />
                            </div>
                        </div>
                    </div>
                    <div className="form-group">
                        <label htmlFor="publicationYear">Ano de Publicação (opcional):</label>
                        <input type="number" id="publicationYear" value={publicationYear} onChange={e => setPublicationYear(e.target.value)} disabled={loading} min="1900" max={new Date().getFullYear()} />
                    </div>
                    ...
                    <div className="form-group">
                        <label htmlFor="content">Conteúdo (resumo/trecho em Markdown, obrigatório):</label>
                        <textarea
                            id="content"
                            value={content}
