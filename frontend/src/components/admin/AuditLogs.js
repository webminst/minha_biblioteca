...
const AuditLogs = () => {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || 'Erro ao carregar dados de auditoria',
      );
    } finally {
      setLoading(false);
    }
  ...
  const generateTestData = async () => {
    try {
      ...
      if (response.data.success) {
        alert(
          `✅ ${response.data.data.generatedLogs} logs de teste gerados com sucesso!`,
        );
                  ...
                  {Object.entries(stats.byAction || {})
                    ...
                    .map(([action, count]) => (
                      <div key={action} className='stat-item'>
                        <span
                          className={`badge badge-${getActionColor(action)}`}
                        >
                          {action}
                        </span>
                ...
                <table className='logs-table'>
                  ...
                  <tbody>
                    {logs.map(log => (
                      <tr
                        key={log.traceId}
                        className={log.response.success ? '' : 'error-row'}
                      >
                        ...
                        <td className='action'>
                          <span
                            className={`badge badge-${getActionColor(log.action.type)}`}
                          >
                            {log.action.type}
                          </span>
                        </td>
                        <td className='resource'>
                          ...
                          {log.action.resourceId && (
                            <small>
                              #{log.action.resourceId.substring(0, 8)}...
                            </small>
                          )}
                        </td>
                        <td className='status'>
                          <span
                            className={`status-badge ${log.response.success ? 'success' : 'error'}`}
                          >
                            {log.response.status}
                          </span>
                        </td>
                        <td className='ip'>{log.request.ip}</td>
                        <td className='duration'>{log.metadata.duration}ms</td>
                        <td className='criticality'>
                          <span
                            className={`badge badge-${getCriticalityColor(log.action.criticality)}`}
                          >
                ...
                {stats.recentActivity.map((activity, index) => (
                  <div key={index} className='activity-item'>
                    ...
                    <div className='activity-content'>
                      <span
                        className={`badge badge-${getActionColor(activity.action)}`}
                      >
                        {activity.action}
                      </span>
                      ...
                      <span
                        className={`activity-status ${activity.success ? 'success' : 'error'}`}
                      >
