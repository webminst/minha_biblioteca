...
jest.mock('../BibleVerse/BibleVerse', () => (props) => <div data-testid="bible-verse">{props.reference}</div>);

import { validateReference, fetchRandomVerse } from '../../services/bibleApiService';
...
describe('BibleSearch', () => {
    ...
    it('busca referência válida e exibe BibleVerse', async () => {
        ...
        render(<BibleSearch />);
        fireEvent.change(screen.getByPlaceholderText(/ex: joão 3:16/i), { target: { value: 'João 3:16' } });
        fireEvent.click(screen.getByRole('button', { name: /buscar/i }));
        await waitFor(() => expect(screen.getByTestId('bible-verse')).toHaveTextContent('João 3:16'));
    });
    ...
    it('alerta se referência inválida', async () => {
        ...
        render(<BibleSearch />);
        fireEvent.change(screen.getByPlaceholderText(/ex: joão 3:16/i), { target: { value: 'Foo' } });
        fireEvent.click(screen.getByRole('button', { name: /buscar/i }));
        await waitFor(() => expect(window.alert).toHaveBeenCalledWith(expect.stringMatching(/não encontrada/i)));
    });
    ...
    it('busca versículo aleatório e exibe BibleVerse', async () => {
        ...
        await waitFor(() => expect(screen.getByTestId('bible-verse')).toHaveTextContent('Salmos 23:1'));
    });
});
