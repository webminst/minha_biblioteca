...
describe('BibleSearchSimple', () => {
  ...
  it('busca versículo e exibe resultado', async () => {
    ...
    render(<BibleSearchSimple />);
    fireEvent.change(screen.getByPlaceholderText(/joão 3:16/i), {
      target: { value: 'João 3:16' },
    });
    fireEvent.click(screen.getByRole('button', { name: /buscar/i }));
    // Garante que o texto do versículo está presente dentro do container correto
    await waitFor(() => {
      const result = screen.getByText(/porque deus amou o mundo/i, {
        selector: '.verse-text',
      });
  ...
  it('busca versículo aleatório e exibe resultado', async () => {
    fetchRandomVerse.mockResolvedValue({
    ...
    await waitFor(() => {
      ...
      const result = screen.getByText(/o senhor é meu pastor/i, {
        selector: '.verse-text',
      });
      expect(result).toBeInTheDocument();
    });
