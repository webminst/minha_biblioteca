...
describe('ContentCard', () => {
    ...
    it('renderiza título, descrição e links', () => {
        ...
        expect(screen.getByRole('link', { name: /ver detalhes/i })).toHaveAttribute('href', '/livros/1');
        expect(screen.getByRole('link', { name: /baixar pdf/i })).toHaveAttribute('href', 'http://exemplo.com/arquivo.pdf');
    });
    ...
    it('renderiza referência para Sermão', () => {
        render(
            <MemoryRouter>
                <ContentCard {...baseProps} type="Sermão" sermon={{ _id: '2' }} reference="Mateus 5:9" />
            </MemoryRouter>
        );
    ...
    it('renderiza referência para Estudo Bíblico', () => {
        render(
            <MemoryRouter>
                <ContentCard {...baseProps} type="Estudo Bíblico" study={{ _id: '3' }} reference="Salmos 23" />
            </MemoryRouter>
        );
