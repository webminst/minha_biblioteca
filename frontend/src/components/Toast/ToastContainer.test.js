...
jest.mock('./Toast', () => ({
    ...
    default: ({ message, onClose, ...props }) => (
        <div role="alert" data-testid="toast" onClick={onClose} {...props}>{message}</div>
    )
}));
...
describe('ToastProvider & ToastContainer', () => {
    ...
    function TestComponent() {
        const { addToast, removeToast, clearToasts, showSuccess, showError, showWarning, showInfo, toasts } = useToast();
        return (
            <div>
    ...
    it('lança erro se useToast for usado fora do provider', () => {
        ...
        expect(() => render(<BadComponent />)).toThrow('useToast deve ser usado dentro de ToastProvider');
    });
});
