...
describe('NewsletterForm', () => {
    ...
    it('renderiza campos e botão', () => {
        ...
        expect(screen.getByPlaceholderText(/seu melhor e-mail/i)).toBeInTheDocument();
        expect(screen.getByRole('button', { name: /assinar/i })).toBeInTheDocument();
    });

    it('valida e-mail vazio ou inválido', async () => {
        render(<NewsletterForm />);
        fireEvent.change(screen.getByPlaceholderText(/seu melhor e-mail/i), { target: { value: '' } });
        ...
        fireEvent.change(screen.getByPlaceholderText(/seu melhor e-mail/i), { target: { value: 'foo' } });
        fireEvent.click(screen.getByRole('button', { name: /assinar/i }));
        expect(await screen.findByText(/insira um e-mail válido/i)).toBeInTheDocument();
    });
    ...
    it('exibe mensagem de sucesso ao inscrever', async () => {
        ...
        render(<NewsletterForm />);
        fireEvent.change(screen.getByPlaceholderText(/seu melhor e-mail/i), { target: { value: 'foo@bar.com' } });
        fireEvent.click(screen.getByRole('button', { name: /assinar/i }));
