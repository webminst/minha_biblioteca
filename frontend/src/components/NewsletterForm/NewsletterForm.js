...
const NewsletterForm = () => {
  ...
  const handleSubmit = e => {
    ...
    const mailchimpUrl =
      'https://ipb.us17.list-manage.com/subscribe/post?u=1ba241b229abed33a57cc2fe2&amp;id=54e21d70a6&amp;f_id=00a846e0f0'.replace(
        '/post?',
        '/post-json?',
      );
        ...
        if (err) {
          ...
          setMessage(
            data.msg.includes('already subscribed')
              ...
              : 'Ocorreu um erro. Verifique seu e-mail.',
          );
  ...
  return (
    <div className='newsletter-form-container'>
      ...
      <p>
        Receba atualizações sobre novos sermões, resumos de livros, estudos e
        outros recursos diretamente no seu e-mail.
      </p>
      ...
      {message && (
        <p
          className={`newsletter-message ${status === 'error' ? 'error' : status === 'success' ? 'success' : ''}`}
        >
          {message}
        </p>
      )}
